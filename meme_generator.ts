# meme_generator.ts
/**
 * 🎭 Meme Generator – Top & Bottom Text (TypeScript Edition)
 * Fully typed, advanced: HTML output, PNG export (optional)
 */
...
function generateHTML(topText: string, bottomText: string, imageUrl: string,
                       fontSize: number = 40, color: string = '#ffffff',
                       output: string = 'meme.html'): void {
...
function exportPNG(htmlFile: string, pngFile?: string): boolean {
...
class MemeApp {
  ...
  private imageUrl: string = DEFAULT_IMAGE;
  ...
  private async setFontSize(): Promise<void> {
  ...
  private async generate(): Promise<void> {
    ...
    generateHTML(this.topText, this.bottomText, this.imageUrl, this.fontSize, this.color, filename);
